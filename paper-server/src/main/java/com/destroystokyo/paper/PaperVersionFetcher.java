...
public class PaperVersionFetcher implements VersionFetcher {
    ...
    private static final ComponentLogger COMPONENT_LOGGER = ComponentLogger
            .logger(LogManager.getRootLogger().getName());
    private static final int DISTANCE_ERROR = -1;
    private static final int DISTANCE_UNKNOWN = -2;
    private static final String DOWNLOAD_PAGE = "https://github.com/Traveling-Trader/CleanPaper/releases";
    private static final String REPOSITORY = "Traveling-Trader/CleanPaper";
    ...
    public Component getVersionMessage() {
        ...
        if (BUILD_INFO.buildNumber().isEmpty() && BUILD_INFO.gitCommit().isEmpty()) {
            updateMessage = text("You are running a development version without access to version information",
                    color(0xFF5300));
        } else {
    ...
    public static void getUpdateStatusStartupMessage() {
        ...
        if (buildNumber.isEmpty() && BUILD_INFO.gitCommit().isEmpty()) {
            COMPONENT_LOGGER
                    .warn(text("*** You are running a development version without access to version information ***"));
        } else {
            ...
            switch (distance) {
                case DISTANCE_ERROR, DISTANCE_UNKNOWN -> {}
                case 0 -> apiResult.ifPresent(result -> {
                    COMPONENT_LOGGER.warn(text(
                            "*************************************************************************************"));
                    COMPONENT_LOGGER.warn(text("You are running the latest build for your Minecraft version ("
                            + BUILD_INFO.minecraftVersionId() + ")"));
                    COMPONENT_LOGGER.warn(text("However, you are " + result.distance()
                            + " release(s) behind the latest stable release (" + result.latestVersion() + ")!"));
                ...
                default -> {
                    if (apiResult.isPresent()) {
                        COMPONENT_LOGGER.warn(text("*** You are running an outdated version of Minecraft, which is "
                                + apiResult.get().distance() + " release(s) and " + distance + " build(s) behind!"));
                        COMPONENT_LOGGER.warn(
                                text("*** Please update to the latest stable version on " + DOWNLOAD_PAGE + " ***"));
                    } else {
                        COMPONENT_LOGGER.info(text("*** Currently you are " + distance + " build(s) behind ***"));
                        COMPONENT_LOGGER.info(text("*** It is highly recommended to download the latest build from "
                                + DOWNLOAD_PAGE + " ***"));
                    }
                }
            }
    ...
    private static Component getUpdateStatusMessage() {
        ...
        return switch (distance) {
            case DISTANCE_ERROR, DISTANCE_UNKNOWN -> text("Checking version...", NamedTextColor.GRAY);
        ...
        try {
            ...
            try (final BufferedReader reader = new BufferedReader(
                ...
                for (final String latestVersion : versionList) {
                    ...
                    try {
                        final URL buildsUrl = URI.create("https://fill.papermc.io/v3/projects/paper/versions/"
                                + latestVersion + "/builds/latest").toURL();
                        ...
                        try (final BufferedReader buildReader = new BufferedReader(
                                new InputStreamReader(connection2.getInputStream(), StandardCharsets.UTF_8))) {
    ...
    private static int fetchDistanceFromSiteApi(final int jenkinsBuild) {
        try {
            final URL buildsUrl = URI.create("https://fill.papermc.io/v3/projects/paper/versions/"
                    + PaperVersionFetcher.BUILD_INFO.minecraftVersionId() + "/builds").toURL();
    ...
    private static int fetchDistanceFromGitHub(final String branch, final String hash) {
        try {
            final HttpURLConnection connection = (HttpURLConnection) URI
                    .create("https://api.github.com/repos/%s/compare/%s...%s".formatted(PaperVersionFetcher.REPOSITORY,
                            branch, hash))
